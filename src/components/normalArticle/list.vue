...
import {fetchNormalArticleList,deleteNormalActivityArticle} from '@/api/article'
export default {
  name: 'ArticleList',
  ...
  methods: {
    ...
    getList() {
      ...
      fetchNormalArticleList(this.listQuery).then(response => {
    ...
    deleteSelectArticle (row) {
      ...
      this.$confirm('确认删除？').then((result) => {
        deleteNormalActivityArticle(articleId).then((result) => {
